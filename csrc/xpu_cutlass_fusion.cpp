      ...
      for (int n = 0; n < N; n++) {
        ...
        for (int l = 0; l < src_loop_num; l++) {
          ...
          for (int v = 0; v < src_vec_size; v++) {
            src_compress_type src_value = reinterpret_cast<sycl::vec<src_compress_type, src_vec_size>*>(cute::raw_pointer_cast(dequant_frag_a.data()))[n*src_loop_num + l][v];
        ...
        for (int l = 0; l < dst_loop_num; l++) {
          reinterpret_cast<sycl::vec<dst_compress_type, dst_vec_size>*>(cute::raw_pointer_cast(mma_B_a.data()))[n * dst_loop_num + l] = reinterpret_cast<sycl::vec<dst_compress_type, dst_vec_size>*>(dst)[l];
        }
      }
    };

    auto dequant_b = [&] (int start_lut_id){
      constexpr int N = decltype(cute::size<1>(mma_B_a))::value;
      constexpr int K = decltype(cute::size(mma_B_a))::value / N;
      ...
      for (int n = 0; n < N; n++) {
        ...
        for (int l = 0; l < src_loop_num; l++) {
          ...
          #pragma unroll
          for (int v = 0; v < src_vec_size; v++) {
            src_compress_type src_value = reinterpret_cast<sycl::vec<src_compress_type, src_vec_size>*>(cute::raw_pointer_cast(dequant_frag_b.data()))[n*src_loop_num + l][v];
            int dst_base_idx = l * src_vec_size * src_compress_size + v * src_compress_size;
            #pragma unroll
            for (int c = 0; c < src_compress_size; c++) {
              uint8_t bit_value = (src_value >> (4 * (((c + 1) & 1) + (c >> 1) * 2))) & 0xF;
        ...
        #pragma unroll
        for (int l = 0; l < dst_loop_num; l++) {
          reinterpret_cast<sycl::vec<dst_compress_type, dst_vec_size>*>(cute::raw_pointer_cast(mma_B_b.data()))[n * dst_loop_num + l] = reinterpret_cast<sycl::vec<dst_compress_type, dst_vec_size>*>(dst)[l];
        }
      }
    };
